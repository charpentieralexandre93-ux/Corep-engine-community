-- =============================================================================
-- 99_bcnf_hardening_v6_6_0.sql
-- Corep Engine Community v6.6.0 — Data Model BCNF Hardening
-- =============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'ck_ref_rule_conditions_operator_v660'
          AND conrelid = 'ref.ref_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_rule_conditions
            ADD CONSTRAINT ck_ref_rule_conditions_operator_v660
            CHECK (condition_operator IN ('=', '!=', '<>', '>', '>=', '<', '<=', 'IN', 'NOT_IN', 'LIKE'))
            NOT VALID;
        ALTER TABLE ref.ref_rule_conditions VALIDATE CONSTRAINT ck_ref_rule_conditions_operator_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_mapping_rule_conditions_operator_v660'
          AND conrelid = 'ref.ref_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_mapping_rule_conditions
            ADD CONSTRAINT ck_ref_mapping_rule_conditions_operator_v660
            CHECK (condition_operator IN ('=', '!=', '<>', '>', '>=', '<', '<=', 'IN', 'NOT_IN', 'LIKE'))
            NOT VALID;
        ALTER TABLE ref.ref_mapping_rule_conditions VALIDATE CONSTRAINT ck_ref_mapping_rule_conditions_operator_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_template_mapping_rule_conditions_operator_v660'
          AND conrelid = 'ref.ref_template_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_template_mapping_rule_conditions
            ADD CONSTRAINT ck_ref_template_mapping_rule_conditions_operator_v660
            CHECK (condition_operator IN ('=', '!=', '<>', '>', '>=', '<', '<=', 'IN', 'NOT_IN', 'LIKE'))
            NOT VALID;
        ALTER TABLE ref.ref_template_mapping_rule_conditions VALIDATE CONSTRAINT ck_ref_template_mapping_rule_conditions_operator_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_rule_conditions_not_blank_v660'
          AND conrelid = 'ref.ref_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_rule_conditions
            ADD CONSTRAINT ck_ref_rule_conditions_not_blank_v660
            CHECK (
        ...
        ALTER TABLE ref.ref_rule_conditions VALIDATE CONSTRAINT ck_ref_rule_conditions_not_blank_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_mapping_rule_conditions_not_blank_v660'
          AND conrelid = 'ref.ref_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_mapping_rule_conditions
            ADD CONSTRAINT ck_ref_mapping_rule_conditions_not_blank_v660
            CHECK (
                NULLIF(TRIM(condition_field), '') IS NOT NULL
                AND NULLIF(TRIM(condition_operator), '') IS NOT NULL
                AND NULLIF(TRIM(condition_value), '') IS NOT NULL
            ) NOT VALID;
        ALTER TABLE ref.ref_mapping_rule_conditions VALIDATE CONSTRAINT ck_ref_mapping_rule_conditions_not_blank_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_template_mapping_rule_conditions_not_blank_v660'
          AND conrelid = 'ref.ref_template_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_template_mapping_rule_conditions
            ADD CONSTRAINT ck_ref_template_mapping_rule_conditions_not_blank_v660
            CHECK (
                NULLIF(TRIM(condition_field), '') IS NOT NULL
                AND NULLIF(TRIM(condition_operator), '') IS NOT NULL
                AND NULLIF(TRIM(condition_value), '') IS NOT NULL
            ) NOT VALID;
        ALTER TABLE ref.ref_template_mapping_rule_conditions VALIDATE CONSTRAINT ck_ref_template_mapping_rule_conditions_not_blank_v660;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_ref_rule_conditions_condition_field_v660'
          AND conrelid = 'ref.ref_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_rule_conditions
            ADD CONSTRAINT fk_ref_rule_conditions_condition_field_v660
            FOREIGN KEY (condition_field)
            REFERENCES ref.ref_condition_fields(condition_field)
            NOT VALID;
        ALTER TABLE ref.ref_rule_conditions VALIDATE CONSTRAINT fk_ref_rule_conditions_condition_field_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_ref_mapping_rule_conditions_condition_field_v660'
          AND conrelid = 'ref.ref_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_mapping_rule_conditions
            ADD CONSTRAINT fk_ref_mapping_rule_conditions_condition_field_v660
            FOREIGN KEY (condition_field)
            REFERENCES ref.ref_condition_fields(condition_field)
            NOT VALID;
        ALTER TABLE ref.ref_mapping_rule_conditions VALIDATE CONSTRAINT fk_ref_mapping_rule_conditions_condition_field_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_ref_template_mapping_rule_conditions_condition_field_v660'
          AND conrelid = 'ref.ref_template_mapping_rule_conditions'::regclass
    ) THEN
        ALTER TABLE ref.ref_template_mapping_rule_conditions
            ADD CONSTRAINT fk_ref_template_mapping_rule_conditions_condition_field_v660
            FOREIGN KEY (condition_field)
            REFERENCES ref.ref_condition_fields(condition_field)
            NOT VALID;
        ALTER TABLE ref.ref_template_mapping_rule_conditions VALIDATE CONSTRAINT fk_ref_template_mapping_rule_conditions_condition_field_v660;
    END IF;
END $$;
...
-- -----------------------------------------------------------------------------
CREATE UNIQUE INDEX IF NOT EXISTS uq_ref_condition_fields_pk_v660
ON ref.ref_condition_fields (condition_field);

CREATE UNIQUE INDEX IF NOT EXISTS uq_ref_mapping_rule_conditions_ordered_v660
ON ref.ref_mapping_rule_conditions (mapping_rule_id, condition_field, condition_operator, condition_value);

CREATE UNIQUE INDEX IF NOT EXISTS uq_ref_template_mapping_rule_conditions_ordered_v660
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_mapping_rules_condition_set_key_v660'
          AND conrelid = 'ref.ref_mapping_rules'::regclass
    ) THEN
        ALTER TABLE ref.ref_mapping_rules
            ADD CONSTRAINT ck_ref_mapping_rules_condition_set_key_v660
            CHECK (condition_set_key ~ '^[0-9a-f]{32,64}$')
            NOT VALID;
        ALTER TABLE ref.ref_mapping_rules VALIDATE CONSTRAINT ck_ref_mapping_rules_condition_set_key_v660;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_ref_template_mapping_rules_condition_set_key_v660'
          AND conrelid = 'ref.ref_template_mapping_rules'::regclass
    ) THEN
        ALTER TABLE ref.ref_template_mapping_rules
            ADD CONSTRAINT ck_ref_template_mapping_rules_condition_set_key_v660
            CHECK (condition_set_key ~ '^[0-9a-f]{32,64}$')
            NOT VALID;
        ALTER TABLE ref.ref_template_mapping_rules VALIDATE CONSTRAINT ck_ref_template_mapping_rules_condition_set_key_v660;
    END IF;
END $$;
...
COMMENT ON VIEW ref.v_ref_mapping_rules_authoring IS
    'Vue de compatibilité v6.6.0 : expose les conditions de mapping normalisées sous forme legacy lisible.';
COMMENT ON VIEW ref.v_ref_template_mapping_rules_authoring IS
    'Vue de compatibilité v6.6.0 : expose les conditions de mapping template normalisées sous forme legacy lisible.';
