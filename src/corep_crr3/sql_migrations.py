...
def discover_plan(sql_root: Path, *, include_reset: bool = False) -> tuple[MigrationStep, ...]:
    ...
    for order, (_, rel_posix, stage_name, sql_path, is_destructive) in enumerate(sorted(candidates), start=1):
        steps.append(
            MigrationStep(
                order=order,
                stage=stage_name,
...
def record_step_sql(
    step: MigrationStep, *, schema: str = MIGRATION_STATE_SCHEMA, table: str = MIGRATION_STATE_TABLE
) -> str:
    ...
    return (
        f"INSERT INTO {qualified}(applied_order, path, sha256, stage) "  # nosec B608
    ...
    for item in records:
        normalized.append(
            AppliedMigration(
                ...
                applied_order=int(item.get("applied_order", -1)),  # type: ignore[call-overload]
            )
        )
