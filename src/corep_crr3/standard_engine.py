...
def ucc_transitional_factor_495d(reporting_date: str | None) -> float:
    """Facteur transitoire Art.495d CRR3 sur le CCF des engagements UCC (bucket 5).

    Par dérogation à l'Art.111(2), le CCF de 10 % des engagements révocables
    sans condition est multiplié par : 0 % jusqu'au 31/12/2029, 25 % en 2030,
    50 % en 2031, 75 % en 2032, 100 % ensuite (Règl. (UE) 2024/1623).
    ``None`` (appels historiques sans date) → 1.0, comportement inchangé.
    """
    if not reporting_date:
        return 1.0
    try:
        year = int(str(reporting_date)[:4])
    except (TypeError, ValueError):
        return 1.0
    if year <= 2029:
        return 0.0
    return {2030: 0.25, 2031: 0.50, 2032: 0.75}.get(year, 1.0)
...
def _maturity_bucket(months: Any) -> Optional[str]:
    """Convertit une maturité en mois en tranche de décote superviseur.

    Tranches de l'Art.224, table 1, CRR3 (Règl. (UE) 2024/1623) : ≤1 an,
    1-3 ans, 3-5 ans, 5-10 ans, >10 ans (granularité affinée en v6.10.0 ;
    l'ancien découpage ≤1/1-5/>5 ans datait du CRR2).
    """
    try:
        m = float(months)
    except (TypeError, ValueError):
...
def _process_standard_exposure(
    ...
    *,
    ...
    if ccf_bucket == "BUCKET_5":
        # Art.495d CRR3 : transitoire UCC (0 % jusqu'à fin 2029, montée 2030-2032).
        ccf *= ucc_transitional_factor_495d(reporting_date)
...
def run_standard_engine(
    ...
    ``reporting_date`` fait partie du contrat public et alimente désormais le
    transitoire UCC de l'Art.495d CRR3 (v6.9.0) ; le schéma normalisé porte la
    date au niveau du batch pour le reste.
    """
    clear_rules_cache()
