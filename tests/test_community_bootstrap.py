...
def _write_contract(sql_dir: Path, paths=("01_first.sql", "02_second.sql")) -> None:
    ...
    (sql_dir / "00_reset_database_dev_ONLY.sql").write_text("DROP SCHEMA IF EXISTS core CASCADE;", encoding="utf-8")
    for path in paths:
        target = sql_dir / path
...
def test_packaged_contract_is_strictly_sa_and_saccr():
    ...
    common_sql = (sql_dir / "01_schema/01_schema_common_community.sql").read_text(encoding="utf-8").lower()
...
def test_bootstrap_rejects_private_engine_content(tmp_path):
    ...
    (tmp_path / "01_first.sql").write_text("CREATE TABLE core.private_cva_results(id integer);", encoding="utf-8")
