...
def test_community_bootstrap_then_sa_and_saccr_engines():
    ...
    try:
        ...
        sa_count = run_standard_engine(db, batch_id, "CRR3_V9", reporting_date, strict_fallback_mode=True)
        ...
        assert db.query("SELECT COUNT(*) AS n FROM ref.ref_mapping_rules WHERE framework <> 'COREP'")[0]["n"] == 0
        assert (
            db.query("SELECT COUNT(*) AS n FROM ref.ref_asset_classes WHERE asset_class_id = 'SECURITISATION'")[0]["n"]
            == 0
        )
    finally:
        db.close()
