...
@pytest.mark.parametrize(
    ("months", "expected"),
    [(None, None), ("bad", None), (12, "≤1Y"), (60, "3Y-5Y"), (61, "5Y-10Y")],  # Art.224 t.1 CRR3 (v6.10.0)
)
...
def test_haircut_lookup_prefers_exact_grade_and_maturity():
    rules = [
        ...
        },
        {
            "collateral_type": "BOND",
            "collateral_grade": "AA",
            "residual_maturity": "3Y-5Y",  # 48 mois -> tranche 3-5 ans (Art.224 t.1 CRR3, v6.10.0)
            "haircut_rate": 0.02,
        },
    ]
    assert (
...
def test_addon_functions_single_bucket_exact_cases():
    ...
    assert math.isclose(
        # Art.280c CRR, table 2 (v6.10.0) : SF crédit HY aligné CQS4 = 1,06 %
        # (était 1,0 %) → 100 000 × 0,0106.
