"""Release contract for the generated Community v6.3.0 source project."""
...
def test_public_product_entry_points_and_version_are_explicit() -> None:
    ...
    assert 'version = "6.3.0"' in pyproject
    assert '__version__ = "6.3.0"' in init_py
    assert readme.startswith("# COREP CRR3 Engine Community — v6.3.0\n")
...
def test_public_branch_baseline_covers_product_surface() -> None:
    ...
    payload = json.loads((ROOT / "evidence/coverage_baseline_v6_3_0.json").read_text(encoding="utf-8"))
    assert payload["product_version"] == "6.3.0"
