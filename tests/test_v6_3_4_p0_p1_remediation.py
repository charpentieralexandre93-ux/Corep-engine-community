"""Non-régression P0/P1 Community v6.10.1."""
...
def test_public_boundary_artifacts_and_metrics_are_current() -> None:
    expected = [
        ROOT / "changelogs/CHANGELOG_v6_10_1.md",
        ROOT / "releases/VALIDATION_v6_10_1.md",
        ROOT / "releases/RELEASE_REPORT_v6_10_1.md",
        ROOT / "SBOM_Corep_Community_v6.10.1.json",
        ROOT / "docs/DATA_MODEL_BCNF_HARDENING_v6_5_0.md",
        ROOT / "evidence/github_ci_proof_checklist_v6_10_1.json",
        ROOT / "evidence/release_metrics_v6_10_1.json",
    ]
    for path in expected:
        assert path.is_file(), path
    for path in [ROOT / "README.md", ROOT / "Dockerfile", ROOT / "docs/COMMUNITY_RELEASE_V6.md"]:
        text = path.read_text(encoding="utf-8")
        assert "6.10.1" in text
        assert "6.3.2" not in text
    metrics = json.loads((ROOT / "evidence/release_metrics_v6_10_1.json").read_text(encoding="utf-8"))
    assert metrics["product_version"] == "6.10.1"
...
def test_community_gui_pure_helpers_cover_boundary_and_validation(tmp_path: Path) -> None:
    ...
    contract.write_text(json.dumps({"version": "6.10.1", "engines": ["SA", "SA_CCR"]}), encoding="utf-8")
...
def test_engine_contracts_and_operational_readiness_paths(tmp_path: Path) -> None:
    ...
    assert report.to_dict()["engine_version"] == "6.10.1"
