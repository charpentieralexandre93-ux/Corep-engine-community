...
def test_v640_resource_budgets_validate_measurements() -> None:
    budgets = resource_budgets.load_budgets(ROOT / "evidence/resource_budgets_v6_4_1.json")
...
def test_v640_regulatory_dossier_is_fail_closed_until_external_evidence() -> None:
    payload = json.loads((ROOT / "evidence/regulatory_dossier_v6_4_1.json").read_text(encoding="utf-8"))
...
def test_v640_active_version_references_are_current() -> None:
    assert (ROOT / "CHANGELOG_v6_4_1.md").is_file()
    assert (ROOT / "VALIDATION_v6_4_1.md").is_file()
    assert (ROOT / "RELEASE_REPORT_v6_4_1.md").is_file()
    assert (ROOT / "evidence/resource_budgets_v6_4_1.json").is_file()
    assert (ROOT / "evidence/regulatory_dossier_v6_4_1.json").is_file()
...
def test_v640_volume_benchmark_evidence_contains_real_stress_profiles() -> None:
    payload = json.loads((ROOT / "evidence/volume_benchmark_v6_4_1.json").read_text(encoding="utf-8"))
...
def test_v640_postgres_explain_budget_gate() -> None:
    ...
    sample = json.loads((ROOT / "evidence/postgres_explain_smoke_v6_4_1.json").read_text(encoding="utf-8"))
    budget = postgres_profiling.load_budgets(ROOT / "evidence/postgres_query_budgets_v6_4_1.json")[0]
