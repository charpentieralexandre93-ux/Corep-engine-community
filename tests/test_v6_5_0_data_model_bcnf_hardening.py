...
def test_v650_current_version_artifacts_are_present() -> None:
    assert (ROOT / "changelogs" / f"CHANGELOG_v{VERSION_U}.md").is_file()
    assert (ROOT / "releases" / f"VALIDATION_v{VERSION_U}.md").is_file()
    assert (ROOT / "releases" / f"RELEASE_REPORT_v{VERSION_U}.md").is_file()
    assert (ROOT / f"docs/DATA_MODEL_BCNF_HARDENING_v{VERSION_U}.md").is_file()
    assert (ROOT / f"releases/evidence/coverage_baseline_v{VERSION_U}.json").is_file()
...
def test_v650_sql_plan_includes_bcnf_hardening_before_final_constraints() -> None:
    ...
    hardening = "04_post_seed/99_bcnf_hardening_v6_5_0.sql"
    assert hardening in manifest
    assert manifest.index(hardening) < manifest.index("99_post_seed_constraints")


def test_v650_evidence_is_version_aligned() -> None:
    payload = json.loads((ROOT / f"releases/evidence/coverage_baseline_v{VERSION_U}.json").read_text(encoding="utf-8"))
