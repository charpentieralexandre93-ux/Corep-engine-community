...
def main() -> int:
    ...
    for required in (
        ROOT / "setup.py",
        ROOT / "MANIFEST.in",
        ROOT / f"CHANGELOG_v{version.replace('.', '_')}.md",
        ROOT / f"VALIDATION_v{version.replace('.', '_')}.md",
        ROOT / f"RELEASE_REPORT_v{version.replace('.', '_')}.md",
        ROOT / f"SBOM_Corep_Community_v{version}.json",
        ROOT / "tools/build_source_archive.py",
        ROOT / "benchmarks/bench_pure_kernels.py",
        ROOT / "evidence/coverage_baseline_v6_2_0.json",
    ):
    ...
    if "max-complexity = 20" not in pyproject:
